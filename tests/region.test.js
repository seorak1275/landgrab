...
test('새 판: 나 + AI 4, 수도는 속초시와 서로 먼 곳, 중립 방어는 생산력 기반, 같은 시드는 같은 판', () => {
  ...
  const c = cap(s, PLAYER); assert.equal(MAP.regions[c.id].n, '속초시'); assert.equal(c.def, 60); assert.equal(c.div, 80); assert.deepEqual(s.run.pool, [300, 100, 100, 100, 100]);
...
test('생산: 세력 풀 하나에 모든 지역 생산이 모이고 한도 500, 전투 중인 지역은 빠진다', () => {
  ...
  near(prodOf(s, c), MAP.regions[c.id].prod); // 시 1.5
  tick(s, 10); near(s.run.pool[PLAYER], 300 + 15);
...
test('유산·축복·난이도: 생산·수비·공격·속도·시작값·중립 감소', () => {
  ...
  near(prodOf(s, c), 1.5 * 1.3); near(defMul(s, c), 1.5 * 1.2); // 속초=강원 산악
...
test('AI: 국경 방어·내부 사단, 유리하면 공격, 집결 주기, 반란 주기', () => {
  ...
  aiAct(s, 1);
  assert.ok(ai.def >= 30 + 60 - 1 && ai.div >= 59 && s.run.pool[1] >= 79, `${ai.def} ${ai.div} ${s.run.pool[1]}`); // 풀의 60%만: 국경 방어 절반·사단 절반, 40%는 남긴다
  ...
  const pc = cap(s, PLAYER); pc.def = 5; pc.div = 5; s.run.pool[1] = 400; ai.div = 0; s.run.aiTurns = [0, REBEL_EVERY - 1]; s.run.elapsed = 1000; // 휴전 끝
  ...
  s.run.rebels = []; s.run.elapsed = 0; s.run.pool[1] = 400; s.run.aiTurns = [0, REBEL_EVERY - 1]; aiAct(s, 1);
