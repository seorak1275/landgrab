...
- [✎ 드래그 선택]을 켜고 내 땅 위를 쭉 그으면 지나간 땅이 전부 선택된다(그 모드에선 지도 이동은 두 손가락). [내 땅 전체 선택]도 있다. 그 뒤 목적지를 탭하면 전부 모아 한 번에 공격/이동.
- 공격은 즉시 끝나지 않고 약 4초 동안 양쪽이 서로 깎이는 전투가 된다(⚔ 표시). 전투 중에 병사를 더 보내면 합류하고, 상대도 수비를 보강할 수 있다. 전투 중인 땅은 징집이 멈춘다.
